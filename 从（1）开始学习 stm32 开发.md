...
[VSCode+EIDE开发STM32，支持标准库、HAL库、LL库，可以在VSCode里进行调试，编译以及下载，代码编辑更舒适，环境搭建超简单。_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV1nr4y1R7Jb/?spm_id_from=333.1387.favlist.content.click&vd_source=7a774f3cabca1af30bf72e095164a42b)

## 工程

导入工程 -> 找到工程绿色图标 -> 添加芯片支持包 -> 设置构建配置 -> 烧录配置：stlink（或 openOCD）

## 调试

直接vscode中进行调试

视图中观察全局变量：加号（add expresion）或双击 + 右键全局变量 -> add to watch

## 编译

快捷键：F7

构建配置 可以选择编译器版本，默认AC5

修改编译优化等级：构建配置 -> 构建器选项（相当于keil中的魔术棒）
...
stm32 中集成了很多用于通信的模块，usart 就是其中最为常用的一个，ppt p91 中列出了 stm32 中的所有通信接口，f103c8t6是全部支持的。
...
USART 的引脚为 `TX（TXD, Transmit exchange 数据发送脚）` 与 `RX（RXD, Receive exchange）数据接收脚` 。接线时注意 TX 接 RX ，且 GND 必须接上。若通信双方都有独立供电则 VCC 可不接。当 *电平标准* 不一致时，还要加电平转换芯片。
...
`UART_HandleTypeDef huart1;`
	串口1的 *句柄（这个东西在hal库编程中十分重要！通常会在跟main.c一个文件夹中的外设.c的开头）* ，包含了跟串口1相关的所有数据。

`HAL_StatusTypeDef HAL_UART_Transmit(UART_HandleTypeDef *huart, const uint8_t *pData, uint16_t Size, uint32_t Timeout)`
	发送数据，参数：句柄， 发送数据， 发送数据的数量（单位为字节），超时时间（单位为 ms， **一般这里填 `HAL_MAX_DELAY`**）。返回数据发送成功（`HAL_OK`）或失败。
	
`HAL_StatusTypeDef HAL_UART_Receive(UART_HandleTypeDef *huart, uint8_t *pData, uint16_t Size, uint32_t Timeout)`
	接收数据（pdata 是指向接收缓冲区的指针）。
