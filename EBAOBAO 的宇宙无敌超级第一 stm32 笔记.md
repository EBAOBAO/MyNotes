...
因为几乎所有的模块都能申请中断，所以如果多个中断发生，就应该有一个东西来管理谁先谁后。将所有中断接到 cpu 上显然会增加 cpu 的负担，故 STM32 使用 NVIC（嵌套中断向量控制器） 外设来统一管理中断，为每个中断通道划分了 16 个可编程的 *优先等级*，像是一个专门为 cpu 安排行程规划处理事件时间的秘书。
