...
2. 器件支持包
3. 软件注册
	1. File -> License Management -> 复制 CID
	2. Keygen 中填入 CID ，Target 选 ARM，生成序列码。
	3. 序列码粘贴到 new LIC
4. stlink驱动
	1. 安装好后的Keil 文件夹 -> ARM -> STLink -> USBDriver 中有安装程序
	2. （JLink 在 ARM -> Seggar 中）
5. usb转串口驱动
	1. 工具文件夹中

# 建立工程

首先新建工程，选择 stm32 的支持包
...
这说的都是什么东西？单片机的编程最基本的方式是直接根据单片机数据手册更改单片机上寄存器的值来实现各种功能，但对于结构复杂的单片机来说这种方式开发也十分复杂，所以厂商往往还会封装出各种库函数来间接配置寄存器，简化开发流程。像标准库跟 HAL 库都是这样的库函数，其中 HAL 库函数还能够被官方的软件用来做到一定程度上的图形化开发，更大地提高了开发效率。

（*固件库在资料中*，库函数本体在Libraries中，Project 中有官方提供的示例跟模板，Utilities 是官方评估板（使用stm32做的一个用于测评stm32的小电路板）相关的东西，接下来还有库函数的发布文档跟使用手册）
...
stm32 的程序从启动文件开始执行。在工程所在文件夹中新建一个目录Start并把这里的启动文件放到里面来进行整理。

外设寄存器描述文件与配置时钟文件：回到 STM32F10x 中，这里的零散的文件也放到启动目录中。

内核寄存器描述文件：Libraries -> CMSIS -> CM3 -> CoreSupport，这些也放在启动目录中。

整理好文件目录后还需要整理kail内部的项目结构来让Keil正确编译并工作：

Source Group 1 改名为 Start，并将启动目录中的文件添加进去：启动文件只能添加一个（这里选md.s），.c 跟 .h 要全部添加 -> 工程选项中添加文件的头文件路径：工程选项（魔术棒） -> C/C++ -> include Paths。

main函数：新建 User 文件夹，main.c 放入其中 -> Target 添加组 -> add new item...

- 右键插入头文件 `#include "stm32f10x.h"` ，
...
工程目录中新建 Library 文件夹，添加库函数：Libraries\\STM32标准外设驱动（STM32F10x_StdPeriph_Driver）\\src、inc -> Target 添加组 Library -> 添加已有文件 -> ctrl + A，add

接着配置库函数头文件包含关系（conf = configuration）以及存放中断函数的文件（it = interrupt）：Project\\STM32F10x_StdPeriph_Template 中 conf 与 it 复制下来到 User -> 配置 Keil
...
说明定义了 `USE_STDPERIPH_DRIVER` 一字符串后 `#include "stm32f10x_conf.h"` 才有效，才能使用固件库。
...
在STM32的固件库中，`GPIO_InitTypeDef` 结构体是不需要动态分配内存的，因为它通常在栈上定义。而且在嵌入式编程中通常不推荐使用动态内存分配，因为它可能会导致内存碎片和其他问题，故在这里你应该新建一个结构体而非动态初始化 一个结构体指针！
...
STM32 中有 68 个可屏蔽中断通道（=中断源，虽然对于一个具体的型号可能没那么多，具体要以对应型号的数据手册为准），几乎所有的模块都能申请（甚至每个模块都能够申请不止一个）中断，像EXTI、TIM、ADC、USART、SPI、I2C、RTC 等外设。
...
正是因为几乎所有的模块都能申请中断，stm32 有一个专门的外设，即 NVIC（Nested Vectored Interrupt Controller，嵌套向量中断控制器），来管理单片机的中断，其管理的方法基本上也就是为所有的这些中断分配 **中断优先级** 了。

## 中断优先级

所以如果多个中断发生，就应该有一个东西来管理谁先谁后。将所有中断接到 cpu 上显然会增加 cpu 的负担，故 STM32 使用 NVIC（嵌套中断向量控制器） 外设来统一管理中断，为每个中断通道划分了 16 个可编程的 *优先等级*，像是一个专门为 cpu 安排行程规划处理事件时间的秘书。

![NVIC](NVIC.png)

这里斜杠加个 n 的意思是一个外设可能占用多个中断通道。

NVIC 中使用 4 位二进制数表示中断优先级，并将优先级信息存储于优先级寄存器中，数值越小表示优先级越高，还可对 它进行分组，进一步设置 *抢占优先级* （与中断嵌套与中断排队有关）与 *响应优先级* （只与中断排队有关）。

![分组](NVICYouXianJi.png)

在一个中断响应函数执行的过程中来了新的中断，这时的第一种解决方法是 *排队* ，对这些所有待执行的中断来说，当然也是优先级越高排队越靠前，若优先级相同则先来后到。

![[paidui.png]]

中断号就是那张表55中左数第二列的数字，也是数值小的优先响应。

NVIC 连接着所有中断，可以控制所有中断的使能和禁止……就是说，中断就像社交软件，已有消息就会发出声音提示你暂且放下手头的事情去处理一下消息，不过你当然也可以在诸如睡觉或干什么重要的事的时候选择让消息静音，这就是“使能”或“禁止”中断。

中断经过 NVIC 的仲裁电路后，胜利的中断需要调用其中断 响应函数，单片机内部有个 flash ，相当于电脑的硬盘，存储着我们所写的所有代码，单片机通过 *中断向量表* 来找到对应的中断函数。
...
数据发送过来后要及时接收，故使用中断接收数据并传到变量中传递，主程序中要用到数据只需调用封装后的 get 函数即可：
...
任何数据内部都是由一个个字节组成的，故从传输时只需只需要使用一个 uint8_t 的指针将数据当成字节数组就行了。
...
通过串口可以让单片机跟一个设备间点对点通信，但要想跟多个设备通信，就要使用 *总线* ，**I2C** 就是一种总线结构，就能让单片机作为主机，其他设备作为从机进行多设备通信（I2C 还可以多主多从通信，但还涉及到总线仲裁什么的，较复杂）。
...
所以我们需要 I2C （Inter IC BUS）通信。

I2C 由两条线组成：SCL（Serial Clock，串行时钟线） 与 SDA（Serial Data，串行数据线）。

![[I2Cjiexian.png]]

之所以都使用开漏输出与上拉电阻，是因为要避免短路，且这样可以实现 *逻辑线与* （单主机情况下 SCL 也可用推挽输出，因为主机对 SCL 要有完全控制）。
...
file -> new project -> commercial part number 填写芯片型号 -> System core -> SYS -> Debug\[Serial Wire\]
...
`UART_HandleTypeDef huart1;` : 串口1的 *句柄（这个东西在hal库编程中十分重要！通常会在跟main.c一个文件夹中的外设.c的开头）* ，包含了跟串口1相关的所有数据。
...
在学习时钟系统之前有必要了解一下单片机的内部结构：单片机就像一台电脑，首先是要有一块主板，然后还需要和购买 cpu、硬盘跟内存条，并把它们都插在主板的 PCIe 上，要是主板没有 usb 接口，就需要使用板卡从 PCIe 上拓展出一些 usb 接口出来然后接 usb hub 再接好多外部设备。

单片机的内部接口及跟它十分相似！其总线矩阵就相当于主板，cortex-m3 就是其 CPU ，flash 就是其硬盘，ram 相当于其内存条，AHB 总线就是其 PCIe 插槽，转换桥就是板卡，APB2 总线相当于一个 usb3.0 hub， APB1 总线则是 usb2.0 hub。

单片机需要给每个模块提供时序信号，并且每个模块需要的时钟的频率是不同的，故需要时钟树来进行时钟信号的产生和分配。

![[clockTree.png]]

这里左边这颗树为芯片上绝大部分模块提供时钟，右边只为 RTC 模块与 IWDG 模块提供时钟。两边的树都有两个树根（两种不同的时钟来源）：（左边的高速树，时钟信号频率一般是几M到几十MHz）HSI 与 HSE 、（右边的低速树，时钟信号频率一般只有几十kHz）LSI 与 LSE 。

- HS - High Speed - 高速
- LS - Low Speed - 低速
- I - Internal - 内部：时钟源在芯片里，硬件成本低，但精度也低。
- E - External - 外部

对我们的最小驱动板来说，高速外部时钟接了一个 8MHz 的晶振，低速外部时钟接了一个 32.768KHz 的晶振。
...
显示单片机所有中断：system core -> NVIC -> show \[all interrupts\] 

中断优先级分组设置：system core -> NVIC -> priority group，然后下方的表格中就可以设置各个中断的优先级，或者在各个外设的选项中也可以设置。

注释中的 PV : Private variables，私有变量，一般将全局变量写在这里

```c
/* Private variables ---------------------------------------------------------*/

/* USER CODE BEGIN PV */
static uint32_t blinckInterval = 1000;
/* USER CODE END PV */
```

USART1 -> NVIC settings -> 勾上 USART1 global interrupt : 开启 USART1 的中断。（或者在 system core -> NVIC 里也行）

`HAL_StatusTypeDef HAL_UART_Receive_IT(UART_HandleTypeDef *huart, uint8_t *pData, uint16_t Size)` : 开始使用中断方式来接收一定数量的数据，接收完之后调用回调函数。

`void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)` : 需要自己写的回调函数，源码 *stm32f1xx_hal_uart.c* 中这个函数前有 `__weak` 关键字（弱方法，可被覆盖），说明它只是声明出来起到一个模板的作用，类似于接口。

用户的自定义函数要在 Private user code 注释下面写。

![中断的实际应用原理](zhongduanUART.png)
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	if (huart == &huart1) // 首先先判断传进来的参数
	{
		...
		HAL_UART_Receive_IT(&huart1, &dataRcvd, 1); // 开启下一个字节的接收
	}
}
/* USER CODE END 0 */
```
