...
这样的代码又为什么是合法且有用的呢？事实上，int 类型是 *值类型* ，这就意味这函数在返回时会返回它的值的完整副本，所以是没问题的。**其实函数返回一个变量时本质上就是返回这个变量所存储的值的副本，但像数组与指针变量存储的值是地址，而风险就在于这些地址的内存可能会随函数执行结束而被释放，导致错误的出现。**

那么该如何返回数组呢？你可以新建一个动态数组然后返回（为什么动态数组可以返回请参阅后文 [[#返回指针]]），或者别在函数中定义数组，这样你要修改数组时甚至不用返回。

**值得一提的是，尽管像结构体、共用体和枚举这样的数据类型看起来很像数组与指针这样的"引用类型"，但它们也是会返回所存储的值的副本的，所以你可以放心地返回这些类型的变量。总之要注意的基本上就只有指针与数组类型。**
