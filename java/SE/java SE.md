...
对象在内存中的存在形式：
- 方法区的常量池中：存放字符串字段内容，加载字段信息与方法信息
...
在Java编程中，主要的变量就是 *字段（成员变量）* 与 *局部变量* ，局部变量一般指方法中定义的变量。

**全局变量**：也就是字段，作用域为整个类体
**局部变量**：除字段外的其他变量，作用域为定义它的代码块中

**全局变量有默认值，局部变量没有**

细节：
- 字段和局部变量可以重名，访问时遵循就近原则
- 在同一作用域中两局部变量不能同名
- 字段随对象的创建而创建，伴随着对象的死亡而死亡；局部变量伴随着它的代码块的执行而创建，伴随着其代码块的结束而死亡。
...
我希望构造器的形参和字段名是一样的（我懒得在这里想什么特殊的名字，而且这样也能使程序更加便于理解），但根据作用域原则，我并不能真么做……

真的没有一个能将对象的字段与方法中的同名局部变量分开，或是让人更加清晰地认识到某一个变量是一对象的字段的方法吗？

**使用`this`就可以做到！**

java虚拟机会给每个对象分配一个 `this`，代表当前对象。**可以认为在对象所被存储的堆内存空间中，有一个隐藏字段 `this` 存储这个对象本身的地址，也就是说 `this` 指向这个对象本身。**

于是，就可以使用 `this` 关键字用来访问本类的字段，方法，构造器了！
...
*封装*
	把抽象出的数据（字段）和对数据的操作（方法）封装在一起数据被保护在内部，程序的其他部分只有通过被授权的操作（方法），才能对数据进行操作。
...
1. 将字段进行私有化
2. 提供一公共（public）set方法，用于对字段判断并赋值。
3. 提供一公共get方法，用于获取字段的值。
...
事实上，私有化字段并提供 getter 与 setter 是相当常见的一种封装的策略，我们专门给用这种方式定义的类一个称呼，叫 *JavaBean* ，一组 getter `getXxx` 与 setter `setXxx` 中的那个 `Xxx` 被我们称为一个 *属性（Property）* 。

（属性是通过`getter`和`setter`来定义的！）
...
*继承* 可以解决代码复用，让我们的编程更加靠近人类思维，当多个类存在相同的字段和方法时，可以从这些类中抽象出父类，在父类中定义这些相同的字段和方法，所有的子类不需要重新定义这些字段和方法，只需通过`extends`声明继承父类即可。
...
子类继承了所有的字段和方法，但是私有字段与方法不能直接在子类里被访问，**如果我们希望字段或方法不被外部访问的同时能够被子类访问，应使用 `protected` 来修饰**。
...
1. 看子类是否有该字段
2. 若有**且可以访问**就返回信息
3. 若没有看父类是否有该字段。无法访问就报错，**不会接着再往上找！！**
4. 重复以上三步直至找到Object，若都没有就报错

### **super**

`super`代表父类的引用。可访问父类的（非private的）字段，方法，构造器。
...
好处：分工明确，父类字段由父类初始化，子类字段由子类初始化。

#### 细节

当子类中有和父类中的成员（字段，方法）重名时，为了访问父类的成员，必须通过 `super`。若无重名，用 `super`，`this`，直接访问都一样。
...
想直接调用字段或方法而不新建对象的话就这样：

```java
((子类类型)父类引用）).方法名()/字段;// 外面的括号是必须的！！！！
```
...
**字段没有重写之说！** 对象的字段的值看它的编译类型。

为什么呢？参考上文**继承的本质**，我们可知对象里是同时存有父类与子类的字段的（包括“被重写过”的），所以对象的编译类型是哪个，它就调用哪个（而对方法来说，实际上也是调用编译类型的方法的，但方法被子类重写过了，也就是说，方法名指向的地址不同了，所以才会调用子类的方法）
...
2. 当调用对象字段的时候，无动态绑定机制，直接访问字段时访问到编译类型中的字段，调用方法中涉及对字段的访问时会调用方法的定义所在类里的字段。
...
用`static`修饰的字段被称为**类变量/静态变量**，它会被所有的类的实例所共享，存放在堆内存或方法区的*静态域*中（取决于jdk版本），各实例中有其变量名指向它本身。
...
4) 创建一个对象时在一个类中的调用顺序：
	1. 静态代码块和静态字段初始化（若有多个，则按它们定义的顺序调用）
	2. 普通代码块与普通字段初始化（若有多个，则按它们定义的顺序调用）
	3. 构造方法
...
**final** 可以用于修饰类、字段、方法和局部变量

**在某些情况下，程序员可能有以下需求，就会用到final：**

1. **当不希望类被继承时，可以用final修饰**
2. **当不希望父类的某个方法被子类重写时，可用final关键字修饰**
3. **当不希望类的某个字段被修改时，可用final关键字修饰**
4. **当不希望某个局部变量被修改时，可用final关键字修饰**

### 细节

1) final修饰的字段又叫常量，一般用XX_XX_XXX这样的形式命名
2) **final修饰的字段在定义时必须赋初值，并且以后不能再修改**，可以在如下任何一个位置赋值：
	1. 定义时
	2. 在构造器中
	3. 在代码块中
...
8) **final和static往往搭配使用，这样能使效率更高！（反正final字段不可被修改，不如让它只在加载类的时候被加载）**
...
5) **抽象类可以有任意成员（抽象类也是类！！），如：非抽象方法、构造器、静态字段……**
...
像usb就是现实的接口，程序里也可以模拟一个接口。它比抽象类还抽象： 如果说让一个类继承一个抽象类是希望它遵循一个模版，那么让一个类实现一个接口就只是希望增加什么功能。即 **如果一个抽象类没有字段，所有方法全部都是抽象方法，就可以把该抽象类改写为接口：`interface`。**
...
6. 接口中的字段都是以 `public static final` 默认修饰的
...
一个类的内部又完整的嵌套了另一个结构，被嵌套的类我们称之为 **内部类（inner class）** ，嵌套其他类的类则称为 **外部类（outer class）** 。内部类最大的特点就是可以直接访问私有字段，并且可以体现类与类的包含关系。注意：*内部类是学习的重难点，后面看底层源码时，有大量的内部类。*
...
class Season {
    ...
    public String getName() { // 2. 去掉set方法，防止字段被修改
        return name;
    }
...
**注解（annotation）** 也被称为 **元数据（metadata）** ，用于修饰解释 包、类、方法、字段、构造器、局部变量等数据信息。简单地说就是放在Java源码的类、方法、字段、参数前的一种特殊“注释”。
...
（它们绝对应该是静态的，因为如果非要new一个实例才能使用它们的话就显得太蠢了）
...
不过要注意，**字符串的字面量从一开始就是对象，而不是这里的要在调用什么方法之后才转换成一个对象，String 不是一个基本类型！！而且字符串的常量池比较特殊，它本身就在堆中！**
...
在堆内存中创建String对象空间，里面维护了 value 字段，常量池中 "EBAOBAO" 的字符内容复制了新的一份到了新的 value 中，s2最终指向的是堆中对象的空间地址。
...
2. **对字符进行处理** ：
	...
	- `int compareTo(String anotherString)`
		比较，原\=\=参数返回0，>参数返回正数，<返回负数，下面是具体计算方法：
		```java
...
2. AbstractStringBuider类有一个没被 `final` 修饰的字段 `char[] value`，因此 StringBuffer 对象的字符串内容是存放在堆中而不是常量池中的。**里面的值可以实际上得以修改，只有空间不够时才开辟新的空间换地址，效率较高。**
...
- LinkedList 实现双向链表与双端队列特点（有`first`与`last`字段），不通过数组存储，添加与删除元素的效率高。可加入`null`，线程不安全，未实现同步。
...
HashSet 找元素时也是用 `equals()` 与 `hashCode()` 方法来找的，故如果你存入过某个对象后再重写它的字段，而后再执行删除等操作时就有可能会出现一些不正常的现象。
...
泛型能够在类声明时通过一个标识表示类中某个字段的类型，或者是某个方法的返回值类型，或参数类型。
...
**线程**
	由进程创建，是进程的一个实体（子任务）。一个线程可以拥有多个进程，且至少会有一个线程。
...
在这里底层使用了一个名为 *代理模式* 的设计模式。其思路主要在于：让一个代理类实现与主题一样的接口，并新建或传入一个主题对象作为代理类对象的字段以调用主题接口相关方法，这样在代理类实现接口的方法时就可以轻松地扩展原主题接口相关方法，或是对主题类实现功能扩展。
...
**注意，要是已输出的类的方法，字段等信息改变了的话，就要重新输出一遍来保证不会出错！！！！**
...
在学习 Java 技术的时候我们就经常会用到 `System.in` 标准输入与 `System.out` 标准输出，它们实际上分别是 InputStream（*当然这是个接口，真正的运行时类型是 BufferedInputStream*） 与 PrintStreram 的字段，标准输入的默认设备就是键盘，而标准输出的默认设备就是显示器。
...
以`String`类为例，当JVM加载`String`类时，它首先读取`String.class`文件到内存，然后，为`String`类创建一个`Class`实例并关联起来（将这个实例赋给类中的 `class` 字段）：
